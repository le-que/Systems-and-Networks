...
pfn_t free_frame(void) {
    //return the frame selected from select_victim_frame()
    pfn_t victim_pfn;
    victim_pfn = select_victim_frame();

    // TODO: evict any mapped pages.
    if (frame_table[victim_pfn].mapped) {
        //update both the page and frame table entries with the appropriate flag
        pte_t* pt = (pte_t*)(mem + ((frame_table[victim_pfn].process->saved_ptbr) * PAGE_SIZE)) + frame_table[victim_pfn].vpn;
        if (pt->dirty) {
            //swap space
            stats.writebacks += 1;
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        if(!last_evicted || last_evicted >= NUM_FRAMES) {
            last_evicted = 1;
        }
        while(1) {
            if (!(frame_table[last_evicted].protected)) {
                last_evicted++;
                return last_evicted - 1;
            }
            last_evicted++;
            last_evicted = last_evicted % NUM_FRAMES;
        }
        ...
        while(1) {
            if (!(frame_table[last_evicted].protected)) {
                pfn_t ret = last_evicted;
                last_evicted = (last_evicted + 1) % NUM_FRAMES;
                 if (!(frame_table[ret].referenced)) {
                    return ret;
                } else {
                    frame_table[ret].referenced = 0;
                }
            } else {
                pfn_t ret = last_evicted;
                last_evicted = (last_evicted + 1) % NUM_FRAMES;
