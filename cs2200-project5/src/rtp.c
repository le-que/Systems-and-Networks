...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    *count = num;
    packets = malloc((unsigned int)num * sizeof(packet_t));
    for (int i = 0; i < num; i++) {
        if (i + 1 < num) {
            ...
            int max = length % MAX_PAYLOAD_LENGTH;
            if (max == 0) {
                max = MAX_PAYLOAD_LENGTH;
            }
            packets[i].payload_length = max;
        ...
        do {
            ...
            */
             if (packet.type == DATA || packet.type == LAST_DATA) {
                packet_t* ack_pack = (packet_t*) malloc(sizeof(packet_t));
